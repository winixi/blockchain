...
import sh.evc.blockchain.util.CryptoUtil;
...
public class BlockService {
  ...
  public String createGenesisBlock() {
    ...
    return JacksonSerialize.beanToJson(genesisBlock);
  }
  ...
  public void replaceChain(List<Block> newBlocks) {
    ...
    if (isValidChain(newBlocks) && newBlocks.size() > localBlockChain.size()) {
      ...
      System.out.println("替换后的本节点区块链：" + JacksonSerialize.beanToJson(blockCache.getBlockChain()));
    } else {
  ...
  public String calculateHash(String previousHash, List<Transaction> currentTransactions, int nonce) {
    return CryptoUtil.SHA256(previousHash + JacksonSerialize.beanToJson(currentTransactions) + nonce);
  }
